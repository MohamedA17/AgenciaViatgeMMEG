<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($nombre_err) && empty($apellido_err) && empty($DNI_err) && empty($pass_err) && empty($cuenta_err) &&  empty($telefono_err) && empty($email_err)){
            ...
            $sql = "INSERT INTO CLIENTE (DNI,NOMBRE,APELLIDO,TELEFONO,CORREO,NUM_TARJETA,PASSWORD) VALUES (?, ?, ?, ?, ?, ?,?)";
            
            if($stmt = mysqli_prepare($link, $sql)){
                mysqli_stmt_bind_param($stmt, "sssssss",$param_DNI,$param_nombre,$param_apellido,$param_telefono,$param_email,$param_cuenta,$param_pass,);
